...
function getGitCommitHash(){
    return childProcess.execSync('git rev-parse HEAD').toString().trim().substring(0, 8);
}

function buildFrontend(){
    return childProcess.execSync('cd windows/desktop && npm install && npm run build');
}
...
version = version.split('.')[0] + "." + yy + "." + (date.getMonth() + 1) + date.getDate() + "." + date.getHours() + date.getMinutes();
