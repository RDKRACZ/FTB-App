          ...
          <div class="tab-pane flex flex-row h-full">
            <div class="mod-list bg-navbar w-1/3 overflow-y-auto relative" ref="modList" id="modList">
              <ul class="p-4" >
                ...
                <div v-if="steps['FINISHED'].done"  class="flex flex-col align-center mx-auto text-center" id="viewInstanceButton" ref="viewInstanceButton">
                  <p>Modpack installed!</p>
                  <ftb-button class="py-2 px-4 mx-2 text-center float-right" color="primary" css-class="text-center text-l"
                            @click="goToInstance()">View Instance</ftb-button>
                </div>
              </ul>
            </div>
            <div class="flex-1 h-full flex flex-col">
              <div class="flex flex-col w-full mt-auto mb-auto" v-if="advertsEnabled">
                <div v-if="!showPlaceholder" id="ow-ad" ref="adRef" ></div>
