          ...
          <span class="instance-info">
            ...
            <div v-if="currentModpack !== null && tags.length > 0" class="flex flex-row items-center">
                <div class="flex flex-row">
                    ...
                    <span v-if="tags.length > 5" :key="`tag-more`"
                          class="rounded mr-2 text-sm bg-gray-600 px-2 lowercase font-light"
                          style="font-variant: small-caps;">+{{tags.length - 5}}</span>
                </div>
            </div>
          </span>
        ...
        <ul class="flex p-2 pb-0" style="position: sticky; top: 0; background: #2A2A2A; padding-bottom: 0;">
          ...
          <li class="-mb-px mr-1">
            <a
                    ...
                    :class="{ 'border-l border-t border-r border-navbar': isTabActive('servers'), 'text-gray-600 hover:text-gray-500': !isTabActive('servers') }"
                    href="#servers"
            >Multiplayer</a>
          </li>
        </ul>
          ...
          <div class="tab-pane" v-if="isTabActive('versions')" id="versions">
            <div v-for="(version, index) in currentModpack.versions" :key="index">
              ...
              <div class="flex flex-row bg-sidebar-item p-5 my-4 items-center">
                ...
                <!--                </button>-->
                <ftb-button v-if="instance.versionId && instance.versionId === version.id"
                            class="py-2 px-4 ml-auto mr-1 cursor-not-allowed" color="primary"
...
export default class InstancePage extends Vue {
    ...
    public launch(): void {
      ...
      const disableChat = (this.settingsState.settings.enableChat === true || this.settingsState.settings.enableChat === "true");
      this.sendMessage({
            payload: {type: 'launchInstance', loadInApp, uuid: this.instance?.uuid, extraArgs: disableChat ? "-Dmt.disablechat=true" : ""},
            callback: (data: any) => {},
        });
    }
