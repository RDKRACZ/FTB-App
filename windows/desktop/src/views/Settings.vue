...
<!--                                    </button>-->
            ...
            <div class="bg-sidebar-item p-5 rounded my-4">
                <div class="flex flex-col my-2">
                  <span>UI Version: {{webVersion}} <font-awesome-icon @click="copyToClipboard(webVersion)" class="ml-2 cursor-pointer" :icon="['fas', 'copy']" size="1x"/><a class="cursor-pointer hover:underline" href="https://github.com/FTBTeam/FTB-App-Overwolf" target="_blank"><font-awesome-icon class="ml-2 cursor-pointer" :icon="['fab', 'github']" size="1x"/></a></span>
...
export default class SettingsPage extends Vue {
    ...
    public browseForFolder() {
        //@ts-ignore
        overwolf.utils.openFolderPicker(this.settingsCopy.instanceLocation, (resp) => {
            if(resp && resp.status === "success"){
                this.settingsCopy.instanceLocation = resp.path;
    ...
    private calcMem(memory: any){
      const total_mem_in_kb = memory/1024;
    ...
    public async uploadLogData(): Promise<void> {
        this.sendMessage({payload: {type: 'uploadLogs', uiVersion: this.webVersion}, callback: async (data: any) => {
            if (!data.error) {
                ...
                //@ts-ignore
                overwolf.utils.placeOnClipboard(url);
                this.showAlert({
                    title: 'Uploaded!',
