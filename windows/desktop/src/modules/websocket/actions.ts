...
export const actions: ActionTree<SocketState, RootState> = {
    sendMessage({commit, rootState}: ActionContext<SocketState, RootState>, payload: MessageData) {
        if(Vue.prototype.$socket.readyState !== 1) {
          console.log("Tried to send message whilst not connected properly", Vue.prototype.$socket.readyState, Vue.prototype.$socket);
          return; 
        }
        ...
        Vue.prototype.$socket.send(JSON.stringify(payload.payload));
