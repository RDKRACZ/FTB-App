...
export const actions: ActionTree<AuthState, RootState> = {
    ...
    async setSessionID({rootState, commit, dispatch}, payload: any): Promise<void> {
        ...
        try {
            response = await axios.get(`https://minetogether.io/api/me`, {headers: {
                'App-Sess-ID': payload, Accept: "application/json"
            },
        ...
        //@ts-ignore
        overwolf.windows.getMainWindow().setAuthData({token: payload, 'app-auth': response['app-auth']});
    ...
    async getNewSession({rootState, commit, dispatch}, payload: any): Promise<void> {
        ...
        //@ts-ignore
        overwolf.windows.getMainWindow().setAuthData({token: response.headers['app-token'], 'app-auth': payload});
        commit('loggedIn');
    },
